...
def base_model():
     model = Sequential()
     model.add(Dense(8, input_dim=7, init='normal', activation='relu'))
...
for i in range(3, l):
    ...
    inten3 = df.loc[i-3,'intensity']
    vel1 = df.loc[i-1, 'velocity']
    vel2 = df.loc[i-2, 'velocity']
    vel3 = df.loc[i-3, 'velocity']
	#tc = df.loc[i-1, 'tweet_count']
    hour = time.hour
    minute = time.minute
    t = hour * 60 + minute
    X.append([t, inten1, vel1, inten2, vel2, inten3, vel3])
X = np.array(X)
scx = MinMaxScaler()
...
print(res)
res.to_csv('Result/AR_ANNreg4_6642.csv', index=False)
